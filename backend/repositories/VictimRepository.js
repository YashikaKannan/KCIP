 ...
 * @description Victim Data Access Repository — Catalyst Data Store implementation
...
import { CatalystVictimRepository } from './catalyst/CatalystVictimRepository.js';
...
export class VictimRepository extends BaseRepository {
  ...
  constructor(catalystApp = null) {
    super();
    this.store = new CatalystVictimRepository(catalystApp);
  }
  ...
  async search(query) {
    if (typeof query === 'string' && !query.includes('=')) {
      return this.store.searchVictims(query);
    }
