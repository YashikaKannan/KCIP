...
import { useAccused } from "@/hooks/api/useKcipQueries";
...
export const Route = createFileRoute("/accused")({
  ...
  component: AccusedPage,
});

function AccusedPage() {
  const { data, isLoading, error } = useAccused();
  ...
  return (
    <>
      ...
      <DataTable
        rows={data ?? []}
        loading={isLoading}
        emptyMessage={error ? "Failed to load accused records" : "No accused records found"}
        columns={[
          { key: "id", header: "ID" },
